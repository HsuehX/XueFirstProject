...
/**
 * Created by Wucn on 2017/6/14.
 */

public class CommonSpinDialog implements View.OnClickListener, AdapterView.OnItemClickListener {

    private Context mContext;
    private AlertDialog mDialog;
    private final int COMPLEXLIST = 3;
...
//    private OnItemClickListener mOnItemTextClickListener;
    ...
    private CommonSpinComplexAdapter mAdapter;
...
//    }
    ...
    public CommonSpinDialog(Context context, int theme) {
        this.mContext = context;
        this.mTheme = theme;
        switch (mTheme) {
            case COMPLEXLIST:
...
//                    mAdapter = new CommonSpinMultipleAdapter(mContext, mList, COMPLEXLIST);
                    mAdapter = new CommonSpinComplexAdapter(mContext, mList, COMPLEXLIST);
...
//    }
    ...
    public void setDataList(ArrayList<SpannerDialogBean> list) {
        if (mList != null) {
            mList.clear();
            mList.addAll(list);
        }
...
//        }
//    }
    public void notifyDataSetAllChanged(List<SpannerDialogBean> list) {
        if (mList != null) {
            mList.clear();
            mList.addAll(list);
            mAdapter.notifiDataSetAllChanged(mList);
...
//            mListOnItemClickListener.onListItemClickListener(mList.get(position), mResIdForListView, position);
            mListOnItemClickListener.onListItemClickListener(mList.get(position).getText(), mResIdForListView, position);
